...
Before we can analyze these data, we first need to read the dataset into R. The 'comma deperated value' (.csv) format is a popular and effective way to export large data sets. This format is a preferred way to store your data, and easily import into R. 
...
We can use the select function in dplyr to choose the columns we want to include for our analyses and plotting. Let's say we weren't interested in the 'ARGOSCLASS' column. We can remove the column from out dataset using dplyr's select() function.
```{r}
...
Up until now, we haven't made any permanent changes to the 'whaleshark' dataframe. We've only demonstrated how certain functions can be used. If we want to make a permanent change, we have to assign the function to a name. We do this using '<-'. 
...
By assigning whaleshark <- before the operations, we have now made a permanent change to the 'whaleshark' dataframe. If you're making lots of changes to your data, you may not want to write over previous versions of your data. But for this demonstration, we will. 
...
ggplot(whaleshark_sf, aes(color = ANIMALID)) + 
  geom_sf() 
```
Amazing! It worked! We can see that shark M-150 has the most detections, and also has the largest spatial range of detections. The data is looking MESSY though. It would be helpful to connect the POINTS with LINES, because then we can visualise each animal's path! 
...
ggplot() +
  ...
  layer_spatial(data = M150_path)
```
Cartolight is a grey marine basemap with societal land features such as city names and roads. Because our shark is in out in the Indian ocean, away from land, this basemap isn't as appropriate for this particular situation. We just want to show you some of the options available! 
...
*So since M150 Path just being one object, does that mean you can't animate it along with the points?*
...
```{r}
library(mapview)
mapview::mapviewOptions(fgb = FALSE) # Quick explanation of this line ******
```

Let's test how it works on the whaleshark_sf 
```{r}

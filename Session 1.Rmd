...
Now that we've loaded our tracking dataset, let's explore the data! 

The easiest way to do this us using pipes, which we can get either from base r in the format |> or %>%, which is riginally from the magrittr package but has been imported to the tidyverse. While both these pipes are *essentially* do the same function, there can sometimes be crucial differences. However, for most coders, the differences are unimportant and you can choose which pipe you prefer. If you're a more advanced coder, you can read about the differences in pipes here: https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/
...
‘Pipes’ the output of the last expression/function (left) forward to the first input of the next function (right).
...
```{r}
# For example, lets say we want to figure out how many detections we have for whale shark 'M-150'...
...
Pipes can also be used to pre-process our data before plotting them. Lets now use pipes to plot a simple plot of number of detections of M-150 over time.
...
Check out this useful online cheatsheet for data wrangling. link: https://rstudio.github.io/cheatsheets/html/data-transformation.html?_gl=1*1osjb37*_ga*OTE3OTY4MTUzLjE2OTMyODk4OTM.*_ga_2C0WZ1JHG0*MTY5MzQ1NDM4NS4yLjAuMTY5MzQ1NDM4NS4wLjAuMA..
...
There is also this awesome cheetsheet for ggplot2 https://rstudio.github.io/cheatsheets/html/data-visualization.html?_gl=1*hd7s9e*_ga*OTE3OTY4MTUzLjE2OTMyODk4OTM.*_ga_2C0WZ1JHG0*MTY5MzQ1NDM4NS4yLjAuMTY5MzQ1NDM4NS4wLjAuMA..
...
Here is a very useful style guide for coding using the sf package. (https://r-spatial.github.io/sf/)

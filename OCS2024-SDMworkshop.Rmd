...
**1. Download the course materials from [this link](https://raw.githubusercontent.com/vinayudyawer/OCS2024_SDMworkshop/main/R/OCS2024-SDMworkshop.zip).**
...
When downloading from **GitHub**, the R console will sometimes ask you if it should also update other packages. In most cases, you can skip updating 
other packages (**option [3]**) as this often stalls the whole process of downloading the package you want. If there are package dependencies, these 
will often be downloaded automatically (but not always!). If you want to update the other packages you have, you can do it separately.
...
This session will address the fundamentals of data pre-processing and visualisation, utilising `lubridate`, `tidyverse`, `sf`, `ggspatial`, and `gganimate` **R** packages. Participants will learn efficient data cleaning methods, organisation of data, and the creation of geospatial visualisations to gain insights into animal distributions and movements.
...
Before we can analyze these data, we first need to read the dataset into **R**. The 'comma separated value' (.csv) format is a popular and effective way to export large data sets. This format is a preferred way to store your data, and easily imported into R. 
...
#First, lets load up lubridate using the library function 
library(lubridate)
...
This may be especially relevant later on when correlating a defined animal behaviour with the time of day.
...
#### Four: Data visualization using `ggplot2`
...
We can solve this by using the `geom_spatial_*()` function from `ggplot2` instead of `layer_spatial`, and using a different type of transition function. In the below example, we provide `gganimate` with the point dataset and use the `transition_reveal()` function instead to animate both the POINT and PATH data. 
...
The package has a clear workflow, and we can walk through the first 4 steps to process and visualise our data, and finally use the package to simulate tracks to use in our subsequent sessions:
...
# Or alternatively, using format_data from the aniMotum Package) 
tagdat2 <- format_data(
  raw_data,
  id = "ANIMALID",
  date = "DATE",
...
fit <-
  fit_ssm(x = tagdat, 
          vmax = 1.5, ## average speed of whale sharks (in m/s)
...
overviewing and conceptialising the objectives of the modelling excercise, and ascertain if this model is relevant for the taxon, location,
...
own SDM to make sure the variables are ecologically relevant to your species. Similarly, pairwise correlations should be conducted to make sure environmental variables
...
complexities in analytic processes, so tread carefully! Here we will go through a ***very*** simple example of using processed satellite tracking data to develop species distribution

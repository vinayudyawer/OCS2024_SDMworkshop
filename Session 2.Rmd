...
raw_data <- read_csv("~/Desktop/Whaleshark.csv")
head(raw_data)
...
ggplot() +
  ...
  geom_spatial_point(data = pred_data, aes(x = lon, y = lat, color = id), crs = 4326) +
  geom_spatial_path(data = pred_data, aes(x = lon, y = lat, color = id), crs = 4326) + 

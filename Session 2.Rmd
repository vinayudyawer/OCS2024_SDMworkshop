...
This is a lot better! When predicting on a 48 hour time step with the pre-filter turned off, the models converged for all sharks. Let's focus on two shark as an example, lets use M-150 and M-130. 
...
ggplot() +
  ...
  labs(title = 'Date: {as.Date(pred_data$date)[frame]}', x = 'Longitude', y = 'Latitude') +
...
# Simulate tracks
The `aniMotum` package also allows users to use your fitted model to simulate similar tracks, which is useful if you want to compare your animal's track with other 'null' models. This allows for further hypothesis testing and analyses like resource selection functions. Here, we will use the `sim()` function to simulate 'null' tracks for all our animals. Lets start with the raw data again to build our state-space model and 
